...
class ConditionalTrainer:
    def __init__(
        ...
        if isinstance(args, dict):
            _args = args
            args = policy.__class__.get_argument(ConditionalTrainer.get_argument())
    ...
    def __call__(self):
        ...
        while total_steps < self._max_steps:
            ...
            if total_steps % self._test_interval == 1:
                avg_test_return, avg_test_steps = self.evaluate_policy(total_steps)
                SINR_1_history, SINR_2_history, transmission_power_1_history, transmission_power_2_history = self._test_env.get_history_data(check_condition=True,
                ...
                average_returns.append(avg_test_return)
        ...
        Path(os.path.join(DATA_STORAGE, self._policy.policy_name)).mkdir(parents=True, exist_ok=True)
        save_algorithm_result(os.path.join(DATA_STORAGE, self._policy.policy_name),average_returns,SINR_1_historys,SINR_2_historys,transmission_power_1_historys,transmission_power_2_historys )
        #np.save(os.path.join(DATA_STORAGE, self._policy.policy_name, "average_returns.npy"), np.array(average_returns))
        #np.save(os.path.join(DATA_STORAGE, self._policy.policy_name, "ue_1_sinr.npy"), np.array(SINR_1_historys))
        #np.save(os.path.join(DATA_STORAGE, self._policy.policy_name, "ue_2_sinr.npy"), np.array(SINR_2_historys))
        tf.summary.flush()
    ...
    def get_argument(parser=None):
        ...
        parser.add_argument('--episode-max-steps', type=int, default=int(1e2),
        ...
        parser.add_argument('--test-interval', type=int, default=int(1e2),
