...
if __name__ == '__main__':
    ...
    #                  color="violet")
    # plt.plot(np.arange(len(bcmq_mean)), bcmq_mean, 'o-', color="violet",
    ...
    #bcmq_mean, bcmq_std = get_reward_history_data("bcmq_lr1e4_batch32_buffer5000")
    #plt.fill_between(np.arange(len(bcmq_mean)), bcmq_mean - bcmq_std,
    #                 bcmq_mean + bcmq_std, alpha=0.1,
    #                 color="gold")
    #plt.plot(np.arange(len(bcmq_mean)), bcmq_mean, 'o-', color="gold",
    #         label="bcmq_lr1e4_batch32_buffer5000")


    bcmq_mean, bcmq_std = get_reward_history_data("bcmq_lr1e4_batch32_buffer1000")
    plt.fill_between(np.arange(len(bcmq_mean)), bcmq_mean - bcmq_std,
                     bcmq_mean + bcmq_std, alpha=0.1,
                     color="g")
    ...
    bcmq_mean, bcmq_std = get_reward_history_data("bcmq_lr1e4_batch32_buffer100")
    plt.fill_between(np.arange(len(bcmq_mean)), bcmq_mean - bcmq_std,
                     bcmq_mean + bcmq_std, alpha=0.1,
                     color="dodgerblue")
